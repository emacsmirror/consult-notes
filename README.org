...
=consult-notes-org-headings-files= will be included in consult-notes. That
variable may be a list of files and/or directories, a single file containing
the list (as with =org-agenda-files=), or a function returning a list of files.
When nil (the default), the current value of =org-agenda-files= is used,
resolved each time the source is consulted.
...
Run =make check= from the repository root to byte-compile all files with
warnings treated as errors and run the ERT test suite. The first run installs
=consult= into a local =.deps= directory; denote and org-roam are stubbed by
the tests, so they need not be installed. =make clean= removes =.deps=.
